...
var (
	addrTmplCreateChatCompletion   = template.Must(template.New("AddressCreateChatCompletion").Funcs(template.FuncMap{"encodejson": openai.EncodeJSON, "trimTrailingSlash": openai.TrimTrailingSlash}).Parse("{{ trimTrailingSlash $.Client.BaseUrl }}/openai{{ if $.Client.DeploymentID }}/deployments/{{ $.Client.DeploymentID }}{{ end }}/chat/completions?api-version={{ $.Client.APIVersion }}"))
	headerTmplCreateChatCompletion = template.Must(template.New("HeaderCreateChatCompletion").Funcs(template.FuncMap{"encodejson": openai.EncodeJSON, "trimTrailingSlash": openai.TrimTrailingSlash}).Parse("Content-Type: application/json\r\nApi-Key: {{ $.Client.APIKey }}\r\n\r\n{{ encodejson $.request }}"))
)

func (__imp *implClient[C]) CreateChatCompletion(ctx context.Context, request *openai.ChatCompletionRequest) (openai.ChatCompletion, error) {
