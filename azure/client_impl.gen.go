...
var (
	addrTmplCreateChatCompletion   = template.Must(template.New("AddressCreateChatCompletion").Funcs(template.FuncMap{"trim_trailing_slash": openai.TrimTrailingSlash}).Parse("{{ trim_trailing_slash $.BaseClient.Endpoint }}/openai/deployments/{{ if $.BaseClient.DeploymentID }}{{ $.BaseClient.DeploymentID }}{{ else }}{{ $.request.Model }}{{ end }}/chat/completions?api-version={{ $.BaseClient.APIVersion }}"))
	headerTmplCreateChatCompletion = template.Must(template.New("HeaderCreateChatCompletion").Funcs(template.FuncMap{"trim_trailing_slash": openai.TrimTrailingSlash}).Parse("Content-Type: application/json\r\nApi-Key: {{ $.BaseClient.APIKey }}\r\n\r\n"))
)
